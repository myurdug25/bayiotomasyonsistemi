...
const shellClass =
  "overflow-hidden rounded-[28px] border border-emerald-300/20 bg-[linear-gradient(145deg,rgba(8,31,24,0.98),rgba(5,20,25,0.98)_55%,rgba(32,46,19,0.96))] shadow-[0_28px_80px_-48px_rgba(16,185,129,0.75)]";
    ...
    <div className="mal-kabul-workspace space-y-4">
      <section className={shellClass}>
        <div className="mal-kabul-summary border-b border-emerald-200/15 px-4 py-4 lg:px-5">
          <div className="grid gap-3 xl:grid-cols-[1fr_1.25fr_1fr_0.85fr_1.3fr_auto] xl:items-end">
        ...
        {receipts.length > 1 ? (
          <div className="flex gap-2 overflow-x-auto border-b border-emerald-200/15 px-4 py-3 lg:px-5">
        ...
        <div className="mal-kabul-panels grid min-h-[520px] xl:grid-cols-[1.55fr_0.95fr]">
          <div className="border-b border-emerald-200/15 p-4 xl:border-b-0 xl:border-r lg:p-5">
            <div className="mb-4 flex items-center justify-between gap-3">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.16em] text-[#f5cf54]">Depolar Arası Transfer ve Mal Kabul</p>
                <h2 className="mt-1 text-xl font-black text-white">Bekleyen Ürünler</h2>
              </div>
              <div className="flex flex-wrap items-center justify-end gap-2">
                <Button
                  type="button"
                  className="h-9 rounded-xl bg-[#f5cf54] px-3 text-xs font-black text-[#211900] hover:bg-[#ffe477]"
                ...
                <Button
                  type="button"
                  variant="outline"
                  className="h-9 rounded-xl border-white/15 bg-white/5 px-3 text-xs font-black text-white hover:bg-white/10"
                ...
                <Badge className="bg-[#f5cf54] px-3 py-1 text-[#241b00]">{waitingItems.length} ürün</Badge>
              </div>
            </div>
            ...
            {receiptsQuery.isLoading ? (
              ...
              <div className="overflow-hidden rounded-[20px] border border-white/10 bg-black/15">
                <div className="hidden grid-cols-[36px_1.45fr_0.9fr_0.9fr_0.65fr_0.7fr_0.65fr_150px] gap-3 border-b border-white/10 bg-emerald-600/20 px-3 py-3 text-[11px] font-black uppercase tracking-[0.08em] text-emerald-50 lg:grid">
                ...
                <div className="divide-y divide-white/10">
                  {waitingItems.map((item) => (
                    <div key={item.id} className="grid gap-3 px-3 py-3 lg:grid-cols-[36px_1.45fr_0.9fr_0.9fr_0.65fr_0.7fr_0.65fr_150px] lg:items-center">
          ...
          <aside className="flex min-h-[480px] flex-col bg-[linear-gradient(180deg,rgba(245,207,84,0.06),rgba(16,185,129,0.04))] p-4 lg:p-5">
            <div className="mb-4 flex items-center justify-between gap-3">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.16em] text-emerald-300">Kabul Listesi</p>
                <h2 className="mt-1 text-xl font-black text-white">Kabul Edilecekler</h2>
              </div>
              <Badge className="bg-emerald-500 px-3 py-1 text-emerald-950">{acceptedItems.length} ürün</Badge>
            </div>
              ...
              {acceptedItems.length ? acceptedItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    className="group grid w-full grid-cols-[36px_1fr_auto] items-center gap-3 rounded-2xl border border-emerald-300/20 bg-emerald-500/10 p-3 text-left transition hover:border-red-300/35 hover:bg-red-500/10"
                  >
            ...
            <div className="mt-4 border-t border-white/10 pt-4">
              <Button
                ...
                className="h-12 w-full rounded-2xl bg-[linear-gradient(135deg,#ff5f62,#dc2626_52%,#991b1b)] text-base font-black text-white shadow-[0_18px_36px_-20px_rgba(239,68,68,0.9)] hover:brightness-110"
              >
                {approveMutation.isPending ? <Loader2 className="h-5 w-5 animate-spin" /> : <Save className="h-5 w-5" />}
                Kaydet
              </Button>
              <p className="mt-2 text-center text-[11px] font-semibold text-white/45">Sağdaki ürünler mevcut Logo mal kabul akışına gönderilir.</p>
            </div>
          </aside>
        </div>
      </section>
...
function InfoField({ label, value, icon }: { label: string; value: string; icon?: ReactNode }) {
  return <div className="min-w-0 rounded-2xl border border-white/10 bg-black/15 px-3 py-2.5"><p className="text-[10px] font-black uppercase tracking-[0.12em] text-white/45">{label}</p><p className="mt-1 flex items-center gap-1.5 truncate text-sm font-black text-white">{icon}{value}</p></div>;
}
...
function Stat({ label, value, tone }: { label: string; value: number; tone: "gold" | "green" | "red" }) {
  ...
  return <div className={`min-w-[76px] rounded-2xl border px-2 py-2 text-center ${classes}`}><p className="text-[9px] font-black uppercase tracking-[0.08em]">{label}</p><p className="mt-1 text-xl font-black">{value}</p></div>;
}
...
function LoadingState() {
  return <div className="flex min-h-52 items-center justify-center gap-2 rounded-2xl border border-white/10 bg-black/10 text-sm font-black text-white/60"><Loader2 className="h-5 w-5 animate-spin" /> Bekleyen ürünler yükleniyor</div>;
}

function EmptyState({ title, compact = false }: { title: string; compact?: boolean }) {
  return <div className={`grid place-items-center rounded-2xl border border-dashed border-white/15 bg-black/10 px-4 text-center ${compact ? "min-h-48" : "min-h-72"}`}><div><PackageCheck className="mx-auto h-9 w-9 text-[#f5cf54]" /><p className="mt-3 text-sm font-black text-white/65">{title}</p></div></div>;
}
